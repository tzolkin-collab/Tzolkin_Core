...
function renderOverviewDashboard(entries,{finance,sales,deploys,infrastructure}={}){
 ...
 $('overview-integrations').replaceChildren(integration('Stripe',sales?.configured?.stripe,`${saleRows.filter(s=>s.provider==='stripe').length} vendas no mês`,providerLogo('stripe')),integration('Asaas',sales?.configured?.asaas,sales?.configured?.asaas?'Leitura por API ativa':'Chave de produção ausente',providerLogo('asaas')),integration('Contas bancárias',bankCount>0,`${bankCount} ${bankCount===1?'conexão bancária':'conexões bancárias'}`),integration('EasyPanel',infrastructure==null?null:infrastructure.status==='ok',infrastructure==null?'Consultando inventário':`${easyCount} serviços no inventário`,providerLogo('easypanel')));
