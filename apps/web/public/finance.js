...
export function setupFinance({api}){
 let generation=0,board=null,sales=null,forecast=null,busy=false,loading=false,message='',error=false,page=0,search='',showForecastForm=false;
 ...
 function salesPanel(){
  ...
  const latest=Object.values(providers).map(p=>p.snapshot?.updated_at).filter(Boolean).sort().at(-1),footer=node('div',undefined,'fin-footer fin-sales-footer');footer.append(node('span',`${rows.length} ${rows.length===1?'venda':'vendas'} no período`),node('span','Última gravação '+timestamp(latest)));section.append(footer);return section;
 }
