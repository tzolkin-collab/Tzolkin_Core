...
export function renderDatabaseWorkspace(root,options){
 ...
 function renderNavigatorList(){
  ...
  list.replaceChildren(...[...groups.entries()].map(([name,items])=>{
   const group=el('div',undefined,'dbx-tree-group'),head=button('',()=>{schema=name;renderNavigatorList();},'dbx-schema-item'+(schema===name?' active':''));head.setAttribute('aria-expanded',schema===name?'true':'false');const relations=tableEdges(tables).filter(edge=>edge.source.schema===name||edge.target.schema===name).length;head.append(el('span',schema===name?'⌄':'›','dbx-tree-chevron'),el('span','◇','dbx-schema-icon'),el('strong',name),el('small',`${items.length} tabelas · ${relations} relações`));
