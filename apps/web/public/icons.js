...
import {BANK_LOGOS,WIDE_LOGOS,BANK_LOGO_FILES} from './finance-model.js';
...
export function createIcon(name){
 ...
 for(const [k,v] of Object.entries({viewBox:'0 0 24 24',width:20,height:20,fill:'none',stroke:'currentColor','stroke-width':1.75,'stroke-linecap':'round','stroke-linejoin':'round','aria-hidden':'true',focusable:'false',class:'delivery-icon'}))svg.setAttribute(k,String(v));
 for(const [tag,attrs] of icons[aliases[name]||name]||icons.layers){const node=document.createElementNS(ns,tag);for(const [k,v]of Object.entries(attrs))node.setAttribute(k,v);svg.append(node);}return svg;
}
...
export function providerLogo(name){
 ...
 // Wordmarks mantêm a proporção; símbolos quadrados usam tamanho compacto.
 const larga=WIDE_LOGOS.includes(name);
 const img=document.createElement('img');img.src='/logos/'+(BANK_LOGO_FILES[name]||name+'.svg');img.alt='';
...
export function paymentInstitutionIcon(){
 const ns='http://www.w3.org/2000/svg',svg=document.createElementNS(ns,'svg');
 for(const [key,value]of Object.entries({viewBox:'0 0 24 24',width:20,height:20,fill:'none',stroke:'currentColor','stroke-width':1.6,'stroke-linecap':'round','stroke-linejoin':'round','aria-hidden':'true',focusable:'false'}))svg.setAttribute(key,String(value));
 const path=document.createElementNS(ns,'path');path.setAttribute('d','M3 9 12 4 21 9Z M5 12v6 M10 12v6 M14 12v6 M19 12v6 M3 21h18');svg.append(path);return svg;
}
