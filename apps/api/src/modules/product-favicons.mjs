...
const iconHref=html=>{
 ...
 for(const link of links){
  ...
  if(href&&/\b(?:icon|shortcut)\b/i.test(rel))return href.trim();
 }
 return null;
};
export function productFaviconRoutes(router){
 router.get('/api/product-favicon',async({url,reply})=>{
  ...
  try{
   ...
   // candidato direto para o browser tentar sem transformar o Core em proxy.
   if(!iconResponse||!icon)return reply(200,{href:new URL('/favicon.svg',response.url||parsed.href).href});
