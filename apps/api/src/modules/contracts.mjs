...
export function contractsRoutes(router) {
 router.put('/api/entitlements', async ({ client, body }) => {
  ...
  // Rascunho aceita contrato preparado: o checklist de ativação o exige antes de ativar.
  await requireProductFor(client, body.product_id, 'access', { draft: true, missing: fail(400, 'Produto não está disponível para contratação.') });
  ...
  await inheritBillingOffer(client,body.tenant_id,body.product_id,body.plan.trim());
