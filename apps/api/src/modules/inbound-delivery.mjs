...
export function inboundDeliveryRoutes(router,{inboundPool}={}) {
 ...
 router.post('/api/commercial/delivery/:id/retry',async({pool,operator,params,reply})=>{
  await commercialPermission(pool,operator,true,true);if(!inboundPool)throw fail(503,'Fila não configurada.');if(!isUuid(params.id))throw fail(400,'Item da fila inválido.');
  const c=await inboundPool.connect();try{await c.query('BEGIN');const changed=await c.query("UPDATE institucional.core_outbox SET status='pending',attempts=0,next_attempt_at=now(),lease_until=NULL,lease_token=NULL,last_error=NULL,updated_at=now() WHERE id=$1 AND status='failed' RETURNING id",[params.id]);if(!changed.rowCount)throw fail(409,'Este item da fila não está em falha.');await c.query("INSERT INTO institucional.core_outbox_audit(outbox_id,action,actor) VALUES($1,'retry',$2)",[params.id,operator.email||operator.subject]);await c.query('COMMIT');}catch(e){await c.query('ROLLBACK');throw e;}finally{c.release();}
