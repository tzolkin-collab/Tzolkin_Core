...
// `isAllowed` deixa o painel conceder acesso sem editar o ambiente.
// A lista do ambiente continua valendo SEMPRE, como quebra-vidro: erro no
// cadastro nunca pode trancar todo mundo para fora da producao.
export function createGoogleIdentity({pool,clientId,clientSecret,publicOrigin,allowedEmails='',isAllowed=async()=>false,clock=Date.now,keySet=GOOGLE_JWKS,fetcher=fetch}={}){
 ...
 return{
  ...
  async finish(callbackUrl){const state=callbackUrl.searchParams.get('state'),code=callbackUrl.searchParams.get('code');if(!state||!code||callbackUrl.searchParams.get('error'))throw Error('Autenticação Google não concluída.');
   ...
   // Ambiente OU cadastro. Falha ao consultar o cadastro nao concede acesso.
   let liberado=emails.has(email);
   if(!liberado){try{liberado=await isAllowed(email)===true;}catch{liberado=false;}}
   if(!liberado)throw Error('Conta Google não autorizada.');
