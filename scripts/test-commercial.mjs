...
// A conexão administrativa abre só para criar e para apagar o banco. Ociosa durante a suíte inteira,
// ela era derrubada pelo servidor e o erro sem ouvinte matava o processo antes do DROP.
const comAdmin=async fn=>{const c=new pg.Client({connectionString:env.DATABASE_URL,connectionTimeoutMillis:10000});c.on('error',()=>{});await c.connect();try{return await fn(c);}finally{await c.end().catch(()=>{});}};
...
} catch(e){console.error({error:e.code||e.message});process.exitCode=1;}finally{await testClient?.end().catch(()=>{});if(created&&/^tzolkin_test_commercial_[0-9a-f]{12}$/.test(name))await comAdmin(c=>c.query(`DROP DATABASE ${name} WITH (FORCE)`));}
