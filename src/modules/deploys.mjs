...
async function consultarProvedor({ name, adapter }) {
 ...
 const resultados = await Promise.all(projetos.map(async projeto => {
  try {
   ...
   return { provider: name, project: projeto.name, git_connected: projeto.git_connected, deployments };
  } catch {
   // Falha em um projeto não derruba os demais; o projeto aparece sem deploys.
   return { provider: name, project: projeto.name, git_connected: projeto.git_connected, deployments: [], partial: true };
  }
 }));
