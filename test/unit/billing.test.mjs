...
const offer={product_id:'commerce',slug:'pro',name:'Plano Pro',provider:'asaas',kind:'one_time',amount_minor:19990,currency:'BRL',interval:null,installments:1,email_owner:'provider',email_templates:{welcome:'boas-vindas'},version:0};
...
test('contract inherits a snapshot; later catalog changes cannot replace it',async()=>{
 ...
 await inheritBillingOffer(client,'tenant','commerce','pro');assert.equal(JSON.parse(queries.at(-1)[1][4]).amount_minor,19990);assert.match(queries.at(-1)[0],/DO NOTHING/);
 let count=0;await inheritBillingOffer({query:async()=>{count++;return{rows:[{offer_slug:'pro'}]};}},'tenant','commerce','pro');assert.equal(count,1);
 await assert.rejects(inheritBillingOffer({query:async()=>({rows:[{offer_slug:'pro'}]})},'tenant','commerce','other'),/revisão contratual/);
});
test('offer version conflict does not overwrite or record a false history entry',async()=>{
 ...
 await assert.rejects(routes.get('/api/billing/offers')({body:{...offer,version:3},client:{query:async(sql)=>{calls++;if(sql.startsWith('SELECT id,name FROM products'))return{rows:[{id:'commerce',name:'Commerce'}]};return{rows:[]};}}}),/outra sessão/);assert.equal(calls,3);
});
test('billing API requires admin and same-origin writes; successful writes are transactional',async()=>{
 const calls=[];const client={query:async(sql)=>{calls.push(sql);return{rows:sql.startsWith('SELECT id,name FROM products')?[{id:'commerce',name:'Commerce'}]:sql.includes('RETURNING version')?[{version:1}]:[]};},release(){}};
 ...
 try{
 assert.equal((await fetch(origin+'/api/billing/offers?product_id=commerce')).status,401);
