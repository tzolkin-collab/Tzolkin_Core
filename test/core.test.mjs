...
test('Core real PostgreSQL security and contract suite',async t=>{
 ...
 const skillerToken=randomBytes(32).toString('base64url');const skillerHash=createHash('sha256').update(skillerToken).digest('hex');const subject=`test:${randomUUID()}`;
 ...
 try{
  ...
  await t.test('ecosystem persisted with three products and no secret fields',async()=>{
  ...
  await t.test('membership of one product does not open another product of the same organization',async()=>{
   ...
   assert.equal((await req('/api/entitlements','PUT',{tenant_id:ids[0],product_id:'skiller',plan:'test',rights:['skills.read'],active:true})).status,200);
   assert.equal((await context(ids[0],skillerToken)).status,403);
   assert.equal((await context(ids[0])).status,200);
   assert.equal((await req('/api/memberships','PUT',{tenant_id:ids[0],product_id:'skiller',subject,active:true})).status,200);
   assert.equal((await context(ids[0],skillerToken)).status,200);
   assert.equal((await req('/api/memberships','PUT',{tenant_id:ids[0],product_id:'skiller',subject,active:false})).status,200);
   assert.equal((await context(ids[0],skillerToken)).status,403);
   assert.equal((await context(ids[0])).status,200);
  });
  ...
  const client=await pool.connect();try{await client.query('BEGIN');await client.query('DELETE FROM app_clients WHERE token_hash=ANY($1::text[])',[[tokenHash,skillerHash]]);for(const table of ['audit_events','memberships','entitlements'])await client.query(`DELETE FROM ${table} WHERE tenant_id=ANY($1::uuid[])`,[ids]);await client.query('DELETE FROM tenants WHERE id=ANY($1::uuid[])',[ids]);await client.query('COMMIT');}finally{client.release();}
